...
import { useCreatePost } from '@/hooks/queries/use-board-queries';
...
interface FeedPostWriteModalProps {
  isOpen: boolean;
...
export function FeedPostWriteModal({
  isOpen,
  onOpenChange,
  boardSlug,
  boardName,
}: FeedPostWriteModalProps) {
  ...
  const createPostMutation = useCreatePost(boardSlug);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Content formatting (markdown/HTML compatibility)
    let finalContent = trimmedText;
    if (uploadedImages.length > 0) {
      // Append images as HTML image tags at the end of the post content
    ...
    const payload = {
      ...
      guestPassword: !user ? guestPassword : undefined,
      allowComments: true,
    ...
    createPostMutation.mutate(payload, {
      ...
      onError: (err: any) => {
        toast.error(err?.message || '글 등록에 실패했습니다.');
      }
    });
  };
  ...
  const handleClose = () => {
    if (content.trim() || uploadedImages.length > 0) {
      if (!confirm('작성 중인 내용이 사라집니다. 정말로 닫으시겠습니까?')) {
        return;
      }
    }
    // Clear states
    ...
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) handleClose(); }}>
      <DialogContent className="max-w-2xl bg-card border border-border p-0 gap-0 overflow-hidden shadow-2xl">
        ...
        <DialogHeader className="px-6 py-5 border-b border-border flex flex-row items-center justify-between">
          <DialogTitle className="text-lg font-bold text-foreground">새 글 쓰기</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 w-full min-w-0 overflow-hidden">
          ...
          {user ? (
            <div className="text-sm font-semibold text-muted-foreground">
              <span className="text-foreground">{user.name}</span>님으로 게시
            </div>
          ) : (
            <div className="flex items-center gap-1.5 text-sm font-medium text-muted-foreground">
              <input
                ...
                placeholder="닉네임"
                className="font-bold border-b border-border/80 focus:border-primary outline-none py-0.5 bg-transparent text-foreground placeholder:text-muted-foreground w-32 text-center"
                maxLength={20}
                autoComplete="off"
              />
              <span>님으로 게시</span>
          ...
          <div className="relative border border-border/80 rounded-2xl p-4 min-h-[160px] flex flex-col focus-within:ring-2 focus-within:ring-blue-400 focus-within:border-blue-400 transition-all bg-card/50 w-full min-w-0 max-w-full">
            <textarea
              ...
              onPaste={handlePaste}
              placeholder="지금 무슨 생각하세요? (이미지 붙여넣기 가능)"
          ...
          <div className="flex items-center justify-end gap-2.5 pt-4 border-t border-border">
            ...
            <Button
              type="submit"
              disabled={createPostMutation.isPending || (content.trim() === '' && uploadedImages.length === 0)}
              className={cn(
